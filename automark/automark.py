...
def test_health_check():
    ...
    check(
        res.status_code == 200, f"Health endpoint expected 200, got {res.status_code}"
    )
    data = res.json()
    check(
...
def test_create_student_persists_and_updates_stats():
    ...
    conn = db_connection()
    cur = conn.cursor()
    cur.execute("SELECT name, course, mark FROM students WHERE id = %s;", (student_id,))
    row = cur.fetchone()
    cur.close()
    conn.close()
...
def test_create_student_without_mark_has_mark_field():
    ...
    check(
        r.status_code == 200,
        f"POST /students (no mark) expected 200, got {r.status_code}",
    )
    check(
        "mark" in body,
...
def test_update_nonexistent_student():
    ...
    check(
        res.status_code == 404,
        f"PUT /students/{missing_id} should return 404, got {res.status_code}",
    )
...
def test_delete_student_updates_stats():
    ...
    students_before = get_students()
    marks_before = [
        s["mark"] for s in students_before if isinstance(s.get("mark"), int)
    ]
    stats_before = get_stats()
    ...
    check(
        res_create.status_code == 200, "Failed to create student for delete-stats test"
    )
    student_id = created["id"]
    ...
    check(
        student_id not in ids_after,
        "Deleted student still appears in /students after DELETE",
    )
...
def test_stats_matches_students_marks():
    ...
    check(
        ...
        f"/stats count {stats['count']} does not match expected {expected_count}",
    )
    check(
        stats["min"] == expected_min,
        f"/stats min {stats['min']} does not match expected {expected_min}",
    )
    check(
        stats["max"] == expected_max,
        f"/stats max {stats['max']} does not match expected {expected_max}",
    )
    check(
        abs(stats["average"] - expected_avg) < 0.1,
        f"/stats average {stats['average']} does not match expected {expected_avg}",
    )
...
def main():
    ...
    tests = [
        ...
        (
            "get_students_structure_and_db_consistency",
            test_get_students_structure_and_db_consistency,
        ),
        (
            "create_student_persists_and_updates_stats",
            test_create_student_persists_and_updates_stats,
        ),
        (
            "create_student_without_mark_has_mark_field",
            test_create_student_without_mark_has_mark_field,
        ),
        (
            "update_existing_student_changes_db_and_response",
            test_update_existing_student_changes_db_and_response,
        ),
        ("update_nonexistent_student_returns_404", test_update_nonexistent_student),
        (
            "delete_existing_student_removes_from_db",
            test_delete_existing_student_removes_from_db,
        ),
